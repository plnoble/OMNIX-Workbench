...
pub fn run_env_diagnostics() -> Result<std::collections::HashMap<String, String>, String> {
    ...
    match std::process::Command::new("node").arg("-v").output() {
        ...
        Err(_) => { map.insert("Node.js".into(), "not found".into()); }
    }
    ...
    match std::process::Command::new("git").arg("--version").output() {
        ...
        Err(_) => { map.insert("Git".into(), "not found".into()); }
    }
    ...
    match std::process::Command::new("rg").arg("--version").output() {
        ...
        Err(_) => { map.insert("Ripgrep".into(), "not found".into()); }
    }
    ...
    for (name, cmds) in [
        ...
        map.insert(name.into(), if found { "✓ installed".into() } else { "not found".into() });
    }
...
pub async fn repair_env_tool(app: tauri::AppHandle, tool_name: String) -> Result<(), String> {
    ...
    let stdout = child.stdout.take().expect("child process stdout was already taken");
    let stderr = child.stderr.take().expect("child process stderr was already taken");
    ...
    for entry in entries.flatten() {
        ...
        match kb_import_file(file_path_str.clone(), None, db.clone()).await {
            ...
            Err(e) => {
                ...
                if !e.contains("文档已存在") {
                    log::warn!("[kb_import_directory] Failed to import {}: {}", file_path_str, e);
                }
            }
        }
    }
