...
工作台接管 DSH 的安装与更新；启动不再全量拉起 9 个 CLI，刷新改为逐个、带进度的探测。

- **DSH 安装/更新（N09，新）**：安装 = 下载官方 NSIS 安装器（dshdesktop.cn，MZ 头校验防
  代理劫持）并拉起可见向导——安装位置与 UAC 由用户确认；更新 = 同一安装器覆盖装。版本比对
  接官方端点，端点异常绝不误报有新版。DSH 的模型与密钥仍全部留在 `~/.dsh`，OMNIX 不代存。
  dsh 不在 PATH 时探测 DSH Desktop 生成的 host-commands 垫片，装完启动一次 DSH Desktop 即可。
- **启动检测轻量化（新）**：打开软件只判「在场」（PATH + 已知位置，零进程）+ 读版本缓存；
  后台只补探一次 DSH（主力优先，带去重闸）。其他 agent 显示缓存版本，点「刷新」才真正重探。
  以前是 9 个 CLI 逐个跑 `--version`（DSH 排最后、要拉起整个 Electron 二进制），又慢又吵。
- **刷新有进度**：逐个探测（DSH 最先），每探完一个发事件，卡片逐个从「检测中」翻到结果，
  刷新按钮实时显示 x/N。不再整排干等一个串行循环，不会再「以为卡住了」。
- **探测健壮性**：每次 `--version` 探测 15 秒上限（超时掐掉，挂住的 CLI 不再卡死整排检测）；
  探测输出全空显示「版本未知」，不再伪造 `0.1.0`。版本缓存存 `agent_cli_versions` 表
  （纯派生数据，不进备份）。
...
- **R01–R04**：平台与首个密钥同事务保存；旧列 `api_key` 不再当真实凭据；编辑平台保留禁用状态。
- **R05**：旧密钥迁移失败不再清空原值。读取、加密写入、校验和清旧列在同一事务；INSERT 失败 / ID 冲突 / 部分迁移整批回滚，原密钥可恢复。告警区分「没有需要迁移的项」和「未完成」。
- **R06**：检查点回退前备份必须成功，否则停止恢复，避免覆盖当前改动。
- **R07**：删平台时同事务清理加密 Key；升级库启动时清孤立记录。
- **R08**：密钥掩码按字符处理，中文 / emoji 不再 panic。
- **R09 / R10**：预览按工作区、模型列表按平台校验请求序号，过期异步结果不回写。
- **R11**：CLI 接管、上游同步、健康检查、知识库嵌入统一走 `platform_keys`，认证头不再发密文。
- **D01–D04**：工作页签恢复最近工作会话；同一任务的文件 / Diff / 日志 / 预览可恢复；交付物与验证分开，测试通过不等于用户接受；Agent 开面板默认不抢焦点，关面板不等于停任务。复用现有会话和 `agent_runs`，没有另起任务系统。

未重建安装包的已安装程序不会自动带上这些修复；本版由 CI 打包发布。

## [0.36.0] - 2026-08-22

换 agent 时的交接从「扔一堆原文」改成「一份交班记录」；超长技能不再是死路。

- **交接改成摘要（新）**：换 agent 时交给接手方的原来是最近 24 轮**原文摘录**
  （8000 字预算）。两个毛病：长对话必然装不下；原文里大部分是过程细节，而接手方
  真正需要的「做到哪了、什么还没解决、下一步干什么」得自己从原文里推。现在先出
  一份三段式摘要。**降级是硬要求**：模型打不通、超时（20 秒）、回空，一律退回原文
  摘录——绝不能因为摘要失败就什么都不给，那样新 agent 是一片空白，而界面上看还是
  一段连续的对话。缓存按「会话 + 最后一条消息 id」，反复交接不重复烧钱，会话推进
  一条就重算。
- **修「长对话交接丢掉的是最新几轮」**：预算截断是从**最老**那头开始累加、超了就停，
  于是被丢掉的是最新的几轮——而接手的 agent 最需要的恰恰是「刚刚发生了什么」。
  24 条 × 单条上限 1200 字最多 28800 字，预算只有 8000，任何真实长对话都必然踩到。
  实测 24 轮长消息时交接块里只有前 7 轮左右。之前没暴露是因为唯一那条测试只有
  3 条短消息，**截断分支从来没被跑到过**。
- **修「超长技能是死路」**：技能注入单条封顶 6000 字，超了截断并留一句「已截断」——
  但注入块里**不给技能所在路径**，模型拿到半份技能没有任何办法读到剩下的。实测某个
  两万多星的开源技能，SKILL.md 有 67 KB，到这里只剩 9%，而它整个设计靠正文里的
  references 链接做渐进披露、那些链接就在截断点之后。现在标头给出源文件路径
  （指向**实际读到的那个文件**，`_core.md` 优先），截断提示改成可操作的。
- **修「设置 → 系统设置 → 文档处理」点进去一片空白**：卡片是有意删掉的（整块是
  布景控件，没有 state、没有后端），但导航项没跟着删。整条导航一起删——文档处理的
  真正入口在「Office」页。顺带核了顶层 24 个应用，都有渲染路径，没有别的空白页。
- **团队跑批：重试加退避、并发拉起加间隔**：worker 失败后原来是立即重试，而多 worker
  跑批最可能的失败就是上游限流（全打同一个订阅）——立即重试必然再撞，重试预算几秒
  烧光。改成 5s / 15s / 45s。并发拉起加 500ms **确定性**间隔（随机抖动是概率性的，
  几个同时就绪的可以全摇到小值照样同时打上游）。
- **重构：抽出共用的切词与命中判定**：记忆召回和技能匹配各写了一份，同一个「按字节
  判长度」的中文分词 bug 因此在两处各自复现过。合并这一层；**各自的领域权重
  （记忆的可信度/证据/工作区、技能的名称/类别/加成）保持不动**。顺带统一了分词的
  长度门槛，取更严的那档（ASCII 词少于 3 个字母跳过——`contains` 是子串匹配，
  `in` 会命中 incident/install/index）。
- 修「只写不读的表」守卫的两个洞（`DELETE FROM x` 被算成读 x；**注释里的 SQL 也被
  算成读**），以及记忆指针块每次改写往文件头多留一个空行。

新增 `docs/设计规约.md`（两条判断：提示词只写形状级规则；已有防护机制就别加第二个）
和 `docs/记忆与交接改造方案.md`（三阶段，已全部实施）。

测试 Rust 657 → 674，前端 78。

## [0.35.1] - 2026-08-21

设置里一个点进去空白的入口，和团队跑批撞限流时的两处硬伤。

- **修「设置 → 系统设置 → 文档处理」点进去一片空白**：那条左侧分组导航有四个组，
  四个组陆续搬走或删掉之后只剩「常规」一个还在渲染，导航整条留着——点「文档处理」
  就没有任何东西。卡片当初是**有意**删的（整块是布景：select 用 defaultValue、
  Input 只有 placeholder、Switch 用 defaultChecked，没有 state 没有后端），删得对，
  但导航项没跟着删。修法是整条导航一起删，不是给它补内容：文档处理的真正入口在
  「Office」页，OfficeCLI 的安装、版本、引擎状态本来就在那里。顺带核了一遍顶层，
  appRegistry 里 24 个应用在 App.tsx 里都有渲染路径，没有别的空白页。
- **团队 worker 重试加退避**：原来失败后是直接重试，中间不等。而多 worker 跑批最
  可能的失败就是上游限流（并发默认 2、上限 4，全打同一个 Claude Code / Codex
  订阅）——立即重试必然再撞一次，重试预算在几秒内烧光，worker 判死。改成
  5s / 15s / 45s，跑满累计 65 秒，和限流冷却一个量级。**不区分失败原因**：agent
  CLI 的错误是非结构化文本，按串嗅探限流很脆，还会多一套要维护的机制。
- **并发拉起加 500ms 确定性间隔**：`.take(concurrency)` 之后是一次性全部 spawn。
  用确定性间隔而不是随机抖动——随机是概率性的，几个同时就绪的可以全都摇到小值，
  照样在同一瞬间打上游；间隔门让突发速率在**构造上**是 1/interval。实现上关键是
  向前预约而不是回头检查：N 个并发调用者各自拿到相隔 500ms 的时隙。重试也走这道
  门（它和首发抢的是同一个订阅）；等待期间状态仍显示 retrying，不假装 running。
- 清掉设置里一整块 `{false && ...}` 的「外观主题」死代码（主题选择器早搬到标题栏）。
  它不害人，但它是唯一让 cn / CardHeader / themeMode 看起来还有人用的地方。

以上两条团队跑批的修正借鉴自 cumora 的 docs/COORDINATION.md（MIT）。

测试 Rust 657 → 660。

## [0.35.0] - 2026-08-20

Auto 选型的三处错、决策留痕、往回翻页的过期响应、合议预设。

- **Auto 选型分类只看最近 4 条，不再扫整段历史**：客户端每轮都把完整历史发上来，
  而分类扫的是全部消息——`need_coding` 被历史里任意一条带三反引号的消息点亮之后
  就**再也变不回 false**；`total_len` 算的是整段对话的长度，聊过两三轮
  `need_speedy` 就永远触发不了。Auto 的省钱效果在长对话里因此归零，而且只单向
  往「能力强」爬。顺带把长度从字节改成**字符**（一个汉字 3 字节，300 的阈值对
  中文实际只有 100 字）。
- **视觉需求改从结构判断**：图片在 Anthropic 那边是独立内容块，而
  `to_string_content()` 只保留 text 块——真图片在文本里根本看不到。原来靠正文里
  出现 "image" 来判断，方向正好是反的：**带图的请求判不出来，正文里提一句
  "docker image" 反而会被判成需要视觉模型**。
- **同分选便宜的 + 防降档**：定价表一直在 `circuit_breaker.rs`（仪表盘和用量统计
  都在读），只有路由这条路没读过它——两个都标了 reasoning 的模型，一个 $0.55/M
  一个 $15/M，原来得分完全相同。防降档是 600 秒窗口内不往更便宜的模型上切（换
  模型会作废上游 prompt cache），但硬性需求（工具、视觉）满足不了时必须换。
- **两条协议路径的路由合成一份**：以前 Anthropic 和 OpenAI 各抄了一份打分，于是
  各自长歪——**「声明了 tools 就只在支持工具的模型里选」这条硬门槛只有 Anthropic
  那边有**，同一个请求走 /v1/chat/completions 就可能被派给一个不会调工具的模型。
- **选型决策留痕（新）**：`router_decisions` 每次 Auto 选型一行，记这一轮需要什么、
  选中谁、费率多少、防降档有没有触发，以及**基线**——不比价、不粘性时会选的那个
  模型。读取端在「用量成本看板」的「Auto 路由」一格。整张表**没有任何自由文本列**，
  prompt 永远进不来，有测试守着这条线。留存 30 天。
- **往回翻页校验相邻性**：等这一页的时候用户切了会话，上一个会话的旧消息会被前置
  进新会话的视图，连「上面还有 X 条」也会被覆盖——两种情况都不报错，只是历史被
  撕成两段。现在过期的一页整页丢掉。
- **合议预设（新）**：两个 Agent 互相看不见地各出一版方案，第三步逐条比对分歧后
  合成，三步全只读。**没做成路由的一档**——融合一轮要跑多次模型，成本方向和省钱
  路由相反；而团队那套调度、依赖透传、审批闸门、验收都是现成的，合议只是它们的
  一种摆法。
- **修好「只写不读的表」守卫的两个洞**：`DELETE FROM x` 被算成读 x（任何自带留存
  清理的表都会被自己的清理语句冒充成有读取端）；**注释里的 SQL 也被算成读**
  ——一句「别写成 ... FROM router_decisions」的解释就够把守卫骗过去。

测试 Rust 633 → 657，前端 74 → 78。
